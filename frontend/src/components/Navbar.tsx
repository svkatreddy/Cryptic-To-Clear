        ...
        <ul className="hidden md:flex items-center gap-1 font-mono text-[13px]">
          {NAV_LINKS.map((link) => (
            <li key={link.href}>
              <Link
                href={link.href}
                className="relative px-4 py-2 rounded-md text-[var(--ink-dim)] hover:text-[var(--ink)] transition-colors group"
              >
                <span className="text-[var(--syn-const)]">/</span>
                {link.label}
              ...
              {NAV_LINKS.map((link) => (
                <li key={link.href}>
                  <Link
                    ...
                    className="block px-3 py-2.5 rounded-md text-[var(--ink-dim)] hover:text-[var(--ink)] hover:bg-white/5 transition-colors"
                  >
                    <span className="text-[var(--syn-const)]">/</span> {link.label}
                  </Link>
                </li>
              ))}
              <li className="pt-2">
