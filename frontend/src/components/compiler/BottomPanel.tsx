...
import { GripHorizontal, CornerDownLeft, Play, Plus, Trash2, CheckCircle2, XCircle, Square } from "lucide-react";
...
export default function BottomPanel({
  ...
  const handleUpdateTestCase = (id: string, field: "input" | "expectedOutput", val: string) => {
    onTestCasesChange?.(
      testCases.map((tc) => (tc.id === id ? { ...tc, [field]: val, status: "idle" } : tc))
    );
  };
  ...
  return (
    <div className="flex h-full flex-col border-t border-[var(--border)] bg-[var(--bg-elevated)] text-[var(--ink)] font-sans">
      ...
      <div
        ...
        className={`flex-1 p-4 font-mono text-[13px] bg-[var(--bg)] overflow-y-auto cursor-text flex-col selection:bg-blue-500/30 select-text ${
      ...
      {/* Test Cases Tab View */}
      <div
        className={`flex-1 p-4 bg-[var(--bg)] overflow-y-auto flex-col font-mono text-[13px] gap-4 ${
          activeTab === "testcases" ? "flex" : "hidden"
        }`}
      >
        ...
        {testCases.length === 0 ? (
          <div className="flex flex-col items-center justify-center p-8 text-center border border-dashed border-[var(--border)] rounded-xl my-auto">
            <p className="text-xs text-[var(--ink-dim)] mb-3">No custom test cases added yet.</p>
            <button
              type="button"
              onClick={handleAddTestCase}
            ...
            {testCases.map((tc, idx) => (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-xs text-[var(--ink-dim)]">Test Case #{idx + 1}</span>
                    {tc.status === "pass" && (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
                  <div>
                    <label className="block text-[11px] text-[var(--ink-dim)] mb-1">Input (STDIN)</label>
                    <textarea
                      value={tc.input}
                  ...
                  <div>
                    <label className="block text-[11px] text-[var(--ink-dim)] mb-1">Expected Output</label>
                    <textarea
                ...
                {tc.actualOutput !== undefined && (
                  <div className="mt-2.5 pt-2.5 border-t border-[var(--border)] text-xs font-mono">
                    <span className="text-[11px] text-[var(--ink-dim)] block mb-1">Actual Output:</span>
      ...
      {/* STDIN Input View */}
      <div
        className={`flex-1 p-3 bg-[var(--bg)] flex-col font-mono text-[13px] ${
          activeTab === "stdin" ? "flex" : "hidden"
        }`}
      >
        <textarea
          value={input}
